...
bool checkUser(std::string username) { // проверка существования пользователя
	for (int i = 0; i < 100; i++) {
...
int main(int argc, char* argv[]) {
	...
	for (int i = 0; i < 100; i++) {
		...
		if (newConnection == 0) {
			...
			Counter++;
			//включаем работу 2 потоков,чтобы в функции main принимались новые соединения , а в процедуре ClientHandler будут ожидаться и отправлятся сообщения клиентам
